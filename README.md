...
Встроенные шаблоны остаются в `summon.html` как fallback. Если настроен `DATABASE_URL`, админка создаёт таблицу `summon_mob_templates` в Neon и сохраняет правки там. Запись с тем же `id`, что у встроенного шаблона, подменяет его для всех пользователей; удаление DB-записи возвращает встроенный вариант.
