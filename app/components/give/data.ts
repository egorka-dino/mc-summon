...
export const ALL_ITEMS = Object.fromEntries(ITEM_GROUPS.flatMap((group) => group.items)) as Record<string, string>;
...
export function enchantsForItem(itemId) {
  if (!itemId) return [];
  if (itemId === 'elytra') return [
    { id: 'unbreaking', name: 'Неразрушимость', max: 3 },
    { id: 'mending', name: 'Починка', max: 1 },
  ];
...
export function canHaveTrim(itemId) {
